...
OO_TOP          = 20     # Máximo de linhas — na prática mostra todos do BESST fora do TOP 10
...
def aplicar_filtros_oportunidades(df, tickers_ranking_top10):
    """
    Retorna TODOS os tickers BESST que não aparecem no TOP 10.

    Sem nenhum filtro de indicadores — o investidor avalia o que vê
    e decide por conta própria quais merecem análise mais profunda.

    Parâmetros:
        df                    : DataFrame completo do Fundamentus
        tickers_ranking_top10 : tickers já no TOP 10 — serão excluídos
    """
    # Todo o universo BESST mapeado, independente de índice
    df = df[df.index.isin(TODOS_TICKERS_BESST)].copy()

    # Remove apenas o que já apareceu na Seção 1
    df = df[~df.index.isin(tickers_ranking_top10)]

    # Converte colunas numéricas para facilitar exibição
    for col in ["p/l", "p/vp", "roe", "cotacao", "dy", "div.liq.patrim"]:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce")

    df["setor"] = df.index.map(get_setor)
    print(f"  {len(df)} tickers BESST fora do TOP 10 encontrados")
    return df
...
def gerar_relatorio_mensal(df_ranking, df_oportunidades,
    ...
    add_par(doc,
            "Esta secao apresenta todos os tickers dos setores de renda passiva "
            "(B.E.S.S.T.) que nao aparecem no ranking principal. Nenhum filtro "
            "de indicadores foi aplicado — o objetivo e dar visibilidade a empresas "
            "que o mercado ainda nao reconheceu ou que estao em momento de "
            "recuperacao. A avaliacao criterios de qualidade fica a cargo do "
            "investidor com base nos dados apresentados.",
            tam=10, ed=6, justificado=True)

    add_par(doc, "Criterio unico aplicado:",
            tam=10, negrito=True, ea=4, ed=2)
    add_par(doc,
            "- Pertence aos setores B.E.S.S.T. e nao aparece no TOP 10 da Secao 1",
            tam=9, recuo=0.5, ed=2)
    add_par(doc,
            "- Ordenacao por P/VP crescente para facilitar identificacao de descontos",
            tam=9, recuo=0.5, ed=6)
    ...
    add_par(doc,
            "Nota: todas as acoes dos setores B.E.S.S.T. ausentes do TOP 10, "
            "sem filtro de indicadores. Ordenadas por P/VP crescente. "
            "A avaliacao da qualidade de cada papel fica a criterio do investidor.",
            tam=8, italico=True,
            cor=RGBColor(0x80, 0x80, 0x80), ea=6)
    ...
    for k, v in [
        ...
        ("Filtro (oportunidades)", "Sem filtros — todos os tickers BESST fora do TOP 10"),
