...
# Filtros das Oportunidades Ocultas (mais flexíveis)
# O objetivo é encontrar empresas sadias que o mercado ainda não precificou
OO_LIQUIDEZ_MINIMA  = 500_000   # Mesmo mínimo de liquidez
OO_ROE_MINIMO       = 0.12      # ROE > 12% — empresa financeiramente sadia
OO_PVPA_MAXIMO      = 2.0       # P/VP <= 2x — não está claramente cara
OO_DIV_PAT_MAXIMO   = 2.5       # Dívida/Patrimônio <= 2,5x — alavancagem razoável
...
def aplicar_filtros_oportunidades(df, tickers_ibov):
    """
    Aplica filtros das Oportunidades Ocultas — mais flexíveis que o ranking.

    Critérios de inclusão (empresa precisa atender TODOS):
    - Pertence ao universo BESST e ao Ibovespa
    - ROE > 12% (empresa financeiramente sadia)
    - P/VP <= 2,0x (não está cara em relação ao patrimônio)
    - Dívida/Patrimônio <= 2,5x (não está excessivamente endividada)
    - P/L > 0 (sem prejuízo)
    - Liquidez >= R$ 500k (operável)

    Sem filtro de DY mínimo — aqui queremos encontrar boas empresas
    independente do yield atual. O CAGR positivo é o que importa.
    """
    df = df[df.index.isin(tickers_ibov)].copy()
    df = df[df.index.isin(TODOS_TICKERS_BESST)]

    for col in ["liq.2meses", "p/l", "p/vp", "roe", "div.liq.patrim"]:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce")

    if "liq.2meses" in df.columns:
        df = df[df["liq.2meses"] >= OO_LIQUIDEZ_MINIMA]
    if "roe" in df.columns:
        df = df[df["roe"] >= OO_ROE_MINIMO]
    if "p/vp" in df.columns:
        df = df[df["p/vp"] <= OO_PVPA_MAXIMO]
    if "div.liq.patrim" in df.columns:
        df = df[df["div.liq.patrim"] <= OO_DIV_PAT_MAXIMO]
    if "p/l" in df.columns:
...
def gerar_relatorio_mensal(df_ranking, df_oportunidades,
    ...
    add_par(doc,
            "Esta secao identifica acoes dos setores de renda passiva com "
            "fundamentos solidos que ainda nao foram plenamente reconhecidas "
            "pelo mercado. O objetivo e encontrar oportunidades onde o investidor "
            "menos experiente nao olha — empresas com ROE elevado, dividendos "
            "crescentes e baixo endividamento, mas que momentaneamente apresentam "
            "DY menor ou sao menos comentadas.",
    ...
    criterios = [
        f"ROE > {OO_ROE_MINIMO:.0%} — empresa financeiramente sadia",
        f"P/VP <= {OO_PVPA_MAXIMO:.1f}x — nao esta claramente cara",
        f"Divida/Patrimonio <= {OO_DIV_PAT_MAXIMO:.1f}x — alavancagem razoavel",
        "P/L positivo — sem prejuizo nos ultimos 12 meses",
        f"Liquidez diaria >= R$ {OO_LIQUIDEZ_MINIMA:,.0f} — operavel",
        "Ordenacao por CAGR de dividendos decrescente",
        "Sem filtro de DY minimo — foco em qualidade e crescimento",
    ]
    ...
    cols_oo = ["#", "Ticker", "Setor", "DY Atual", "CAGR Div",
               "ROE", "P/VP", "P/L", "Div/Patrim"]
    tab_oo = doc.add_table(rows=1, cols=len(cols_oo))
    tab_oo.alignment = WD_TABLE_ALIGNMENT.CENTER
    for i, c in enumerate(cols_oo):
        cel_cabecalho(tab_oo.rows[0].cells[i], c)

    for pos, (ticker, row) in enumerate(
            df_oportunidades.head(OO_TOP).iterrows(), 1):
        # Busca CAGR do histórico se já calculado, senão coloca N/D
        cagr_v = df_hist_ind.loc[ticker, "cagr_div"] \
                 if ticker in df_hist_ind.index else float("nan")
        cagr_s = f"{float(cagr_v):.1%}" \
                 if not math.isnan(float(cagr_v)) else "N/D"
        ...
        vals  = [
            ...
            f"{row['p/vp']:.2f}"  if pd.notna(row.get("p/vp"))else "N/D",
            f"{row['p/l']:.1f}"   if pd.notna(row.get("p/l")) else "N/D",
            f"{dvp_v:.2f}x"       if pd.notna(dvp_v)          else "N/D",
        ]
        for i, v in enumerate(vals):
            cor = None
            if i == 4:  # CAGR
    ...
    add_par(doc,
            "Nota: empresas ordenadas por CAGR de dividendos decrescente. "
            "DY abaixo de 6% nao indica fraqueza — pode refletir reinvestimento "
            "estrategico ou momento de ciclo favoravel de longo prazo.",
            tam=8, italico=True,
            cor=RGBColor(0x80, 0x80, 0x80), ea=6)
    ...
    for k, v in [
        ...
        ("Filtro DY (oportunidades)",   "Sem filtro de DY"),
        ("Filtro BESST",                "Ativo"),
    ]:
        p = doc.add_paragraph()
        p.paragraph_format.space_after = Pt(2)
...
if __name__ == "__main__":
    ...
    df_oo_filtrado = aplicar_filtros_oportunidades(df_todos, tickers_ibov)
    ...
    # Ordena oportunidades ocultas por CAGR decrescente
    # Cruza com df_hist_ind para ter o CAGR disponível na ordenação
    ...
    # Ordena: CAGR positivo primeiro, depois pelo maior CAGR
    df_oo_filtrado = df_oo_filtrado.sort_values(
        "cagr_div", ascending=False, na_position="last"
    )
