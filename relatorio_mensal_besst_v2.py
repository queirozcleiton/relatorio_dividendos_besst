...
OO_PVPA_MAXIMO  = 1.0    # P/VP <= 1,0x — negocia abaixo do patrimônio
...
def aplicar_filtros_oportunidades(df, tickers_ibov):
    ...
    Filosofia: encontrar empresas BESST desvalorizadas onde é possível
    acumular volume com baixo desembolso por ação. O preço nominal
    baixo, combinado com P/VP abaixo de 1,0 (negociando abaixo do
    patrimônio), indica que o mercado está subprecificando a empresa
    em relação ao que ela realmente vale no balanço.

    Critérios aplicados:
    - Pertence ao universo BESST e ao Ibovespa
    - ROE > 8%  — empresa ainda operacionalmente sadia
    - P/VP <= 1,0x — negocia abaixo do valor patrimonial
    - P/L > 0  — sem prejuízo nos últimos 12 meses
    - Sem filtro de liquidez — estratégia de acumulação e manutenção
    - Sem filtro de DY mínimo — foco em valor, não em yield imediato
    - Sem filtro de dívida — evita restringir demais o universo pequeno
    """
    df = df[df.index.isin(tickers_ibov)].copy()
    df = df[df.index.isin(TODOS_TICKERS_BESST)]
...
def gerar_relatorio_mensal(df_ranking, df_oportunidades,
    ...
    criterios = [
        f"P/VP <= {OO_PVPA_MAXIMO:.1f}x — negocia abaixo do valor patrimonial",
        ...
        "Sem filtro de DY minimo — foco em valor, nao em yield imediato",
    ...
    add_par(doc,
            "Nota: empresas ordenadas por P/VP crescente — as mais descontadas "
            "em relacao ao patrimonio aparecem primeiro. O preco nominal (Cotacao) "
            "e apresentado como referencia para planejamento de aporte, "
            "mas nao e criterio de qualidade. P/VP verde indica desconto "
            "expressivo (abaixo de 0,50x); amarelo indica desconto moderado.",
            tam=8, italico=True,
            cor=RGBColor(0x80, 0x80, 0x80), ea=6)
